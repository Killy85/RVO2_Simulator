...
namespace RVO
{
    ...
    internal class KdTree
    {
        ...
        internal void buildObstacleTree()
        {
            ...
            IList<Obstacle> obstacles = new List<Obstacle>(sim_.obstacles_.Count);

            for (int i = 0; i < sim_.obstacles_.Count; ++i)
            {
                obstacles.Add(sim_.obstacles_[i]);
            }
...

            {
                ...
                for (int j = 0; j < obstacles.Count; ++j)
                {
                    ...
                    else
                    {
                        ...
                        newObstacle.id_ = sim_.obstacles_.Count;

                        sim_.obstacles_.Add(newObstacle);
